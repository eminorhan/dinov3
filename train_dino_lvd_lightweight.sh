...
#SBATCH --time=6:00:00
#SBATCH --job-name=train_lvd_lightweight
#SBATCH --output=train_lvd_lightweight_%A_%a.out
#SBATCH --array=0
...
export NCCL_PXN_DISABLE=1  # need this for proper checkpointing for some reason.
...
CONFIG_FILE=${CONFIG_FILE:-"./dinov3/configs/train/oo_2d_l_lvd_lightweight.yaml"}

srun torchrun --nnodes $SLURM_NNODES --nproc_per_node $GPUS_PER_NODE --max_restarts 1 --node_rank $SLURM_NODEID --rdzv_id 101 --rdzv_backend c10d --rdzv_endpoint "$MASTER_ADDR:$MASTER_PORT" ./dinov3/train/train.py --config-file ${CONFIG_FILE}
